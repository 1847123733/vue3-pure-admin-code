...
import { isString, cloneDeep, isAllEmpty, intersection, storageSession } from '@pureadmin/utils';
...
function initRouter() {
  return new Promise(resolve => {
    const data = getToken();
    getAsyncRoutes({ userid: data.userId }).then(res => {
      if (res.code == 200) {
        //将扁平结构转为树形结构,去掉类型为BUTTON的字段单独存储做按钮权限控制
        const list = cloneDeep(res.data.permissionTreeVO.rows);
        const fineGritPermission = list.filter(item => item.type === 'BUTTON' && item.visibility).map(x => x.url);
        usePermissionStoreHook().initFineGritPermission(fineGritPermission);
        const pageList = list
          .filter(item => item.type !== 'BUTTON' && item.visibility)
          .map(item => {
            return {
              path: item.url,
              id: item.id,
              ...
              rank: item.sort,
              type: item.type,
              showLink: item.type != 'HIDE_MENU'
            };
          });
        const tree = listToTree(pageList);
        treeForeach(tree, function (node) {
          let icon: string = node.icon;
          if (node.icon && node.icon.startsWith('el-icon-')) {
            //兼容老板的图标
            icon = node.icon.replace('el-icon-', 'ep:');
          }
          node.meta = {
            title: node.title,
            icon: icon,
            ...
            keepAlive: true,
            showParent: true,
            showLink: node.showLink,
            type: node.type
          };
          // node.path = node.url;
          // node.redirect = node.url;
          ...
          delete node.title;
          delete node.rank;
          delete node.showLink;
          // if (node.children) {
          //   delete node.name;
          // }
          node.name = node.path.split('/').at(-1);
          if (node.children.length === 0) {
            delete node.children;
            delete node.meta.rank;
          }
        });
        handleAsyncRoutes(cloneDeep(tree));
        resolve(router);
      } else {
...
function addAsyncRoutes(arrRoutes: Array<RouteRecordRaw>) {
  ...
  arrRoutes.forEach((v: RouteRecordRaw) => {
    ...
    if (['HIDE_MENU', 'INNER_MENU'].includes(v.meta.type as string)) {
...
function hasAuth(value: string | Array<string>): boolean {
  ...
  const isAuths = isString(value) && usePermissionStoreHook().hasFineGritPermission(value);
  return isAuths;
}
