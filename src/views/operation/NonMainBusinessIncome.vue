...
import { TotalAmount } from '@/api/operation/nonMainBusinessIncome';
...
defineOptions({
  name: 'NonMainBusinessIncome' //非主营业务收入统计
});
...
const state = reactive({
  tableData: [],
  rowData: {} as TotalAmount
});
const queryLoading = ref(false);
const getList = async () => {
  queryLoading.value = true;
  const res = await serve.queryList({
    ...queryParam,
...
const getListTotal = async () => {
  queryLoading.value = true;
  const res = await serve.getTotalAmount({
    ...queryParam,
    stateFlag: queryParam.stateFlag.length == 6 ? undefined : queryParam.stateFlag.join(',')
  });
  queryLoading.value = false;
  ...
  <main>
      ...
      <el-row :gutter="20">
        ...
        <el-col :xl="55" :lg="8">
          <el-button-group>
            ...
            <base-download-button url="/storeOtherIncome/exportList" :param="queryParam">导出</base-download-button>
      ...
      <base-table-bar-box>
        <template #button>
          <el-button
            ...
            type="primary"
            :icon="Upload"
            v-auth="'/operation/NonMainBusinessIncomeChannel'"
            @click="openUpload"
        ...
        <template #statistics>
          <div class="statistics-box">
            <span>充电宝合计： {{ state.rowData?.totalPowerBank || 0 }}</span>
            <span>打印机合计： {{ state.rowData?.totalPrinter || 0 }}</span>
            <span>其他合计： {{ state.rowData?.totalOther || 0 }}</span>
          </div>
        </template>
      </base-table-bar-box>
    </el-form>
    <el-row>
      ...
      >
        ...
        <el-table-column label="充电宝(元)" prop="powerBank" min-width="120" />
        <el-table-column label="打印机(元)" prop="printer" />
        <el-table-column label="其他业务(元)" prop="other" />
    ...
    <base-upload-dialog
      :isShow="isShow"
      :limitAmount="1"
      uploadUrl="/storeOtherIncome/uploadFile"
      templateDownloadUrl="/common/templateDownLoad"
      :paramDownloadUrl="{
        templateEnum: 'STORE_OTHER'
      }"
