...
// useDark
import { debounce, useGlobal, storageLocal, storageSession } from '@pureadmin/utils';
...
// themeColors   setLayoutThemeColor  dataTheme
...
const settings = reactive({
  ...
  pageChangeAnimation: $storage.configure.pageChangeAnimation
});
...
const pageChangeAnimationChange = () => {
  const pageChangeAnimation = settings.pageChangeAnimation;
  storageConfigureChange('pageChangeAnimation', pageChangeAnimation);
  usePageChangeAnimationStoreHook().setPageChangeAnimation();
};
...
<template>
  <panel>
    ...
    <ul class="setting">
      ...
      <li>
        <span class="dark:text-white">页面切换动画</span>
        <el-switch
          v-model="settings.pageChangeAnimation"
          inline-prompt
          inactive-color="#a6a6a6"
          active-text="开"
          inactive-text="关"
          @change="pageChangeAnimationChange"
        />
      </li>
