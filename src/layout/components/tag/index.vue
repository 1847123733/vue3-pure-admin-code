...
function deleteDynamicTag(obj: any, current: any, tag?: string) {
  ...
  if (current === route.path) {
    ...
    if (tag === 'left') return;
    if (newRoute[0]?.query) {
      router.push({ path: newRoute[0].path, query: newRoute[0].query });
    } else if (newRoute[0]?.params) {
      router.push({ path: newRoute[0].path, params: newRoute[0].params });
    } else {
...
function tagOnClick(item) {
  ...
  if (item.query) {
    routeObj.query = item.query;
  }
  // if (item.params) {
  //   routeObj.params = item.params;
  // }
