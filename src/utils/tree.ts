 ...
 * @return {*}
 */
export function listToTree(list, parentKey = 'parentId', childKey = 'children') {
  const info = list.reduce((map, node) => ((map[node.id] = node), (node[childKey] = []), map), {});
  return list.filter(node => {
    info[node[parentKey]] && info[node[parentKey]][childKey].push(node);
