  ...
  actions: {
    ...
    handleTags<T>(mode: string, value?: T | multiType, position?: positionType): T {
      switch (mode) {
        ...
        case 'replace': {
          const tagVal = value as multiType;
          const index = this.multiTags.findIndex(v => v.path === tagVal.path);
          if (index === -1) return;
    ...
    handleRouterAndTags(
      ...
      if (redirectType === 'replace' && this.multiTags.some(x => x.path === path)) {
        this.handleTags('replace', {
          path,
          query,
          meta: {
            title
